...
set<int> luby_algorithm_blocked_assignment_async(int procID, int nproc, int n, int E, set<int> * adj_list) {
    ...
    double total_time_step4a = 0;
    double total_time_step4b = 0;
    ...
    while (true) {
        ...
        float neighbor_vals[edges];
        int u_vals[edges];
        MPI_Request neighbor_reqs[edges];
        int waiting_on = 0;
        ...
        for (int i = 0; i < edge_list.size(); i++) {
            ...
            if (v_process == procID && u_process == procID) {
                ...
                MPI_Isend(&communicated_priority, 1, MPI_FLOAT, v_process, u, MPI_COMM_WORLD, &send_request);
                u_vals[i] = u;
                MPI_Irecv(&neighbor_vals[i], 1, MPI_FLOAT, v_process, v, MPI_COMM_WORLD, &neighbor_reqs[i]);
                waiting_on ++;
            }
        }
        time_inc = MPI_Wtime() - time_inc_start;
        total_time_step4a += time_inc;
        ...
        for (int i = 0; i < edge_list.size(); i++) {
            ...
            int u_process = u / vertices_per_process;
            int v_process = v / vertices_per_process;
            if (u_process == procID && v_process == procID) {
                continue;
            } else {
                MPI_Wait(&neighbor_reqs[i], MPI_STATUS_IGNORE);
                // MPI_Recv(&neighbor_vals[i], 1, MPI_FLOAT, v_process, v, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                int u = u_vals[i];
                max_priority_of_neighbors[u - start] = max(max_priority_of_neighbors[u - start], neighbor_vals[i]);

            }
        }

        // int received = 0;
        // while (received < waiting_on) {
        //     int index;
        //     MPI_Waitany(edges, neighbor_reqs, &index, MPI_STATUS_IGNORE);
        //     printf("INDEX %d\n", index);
        //     int u = u_vals[index];
        //     max_priority_of_neighbors[u - start] = max(max_priority_of_neighbors[u - start], neighbor_vals[index]);
        //     received += 1;
        // }

        time_inc = MPI_Wtime() - time_inc_start;
        total_time_step4b += time_inc;
    ...
    printf("Process: %d, time for step %da: %f\n", procID, 4, total_time_step4a);
    printf("Process: %d, time for step %db: %f\n", procID, 4, total_time_step4b);
